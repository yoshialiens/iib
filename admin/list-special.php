<?php
	...
	if($_SERVER['REQUEST_METHOD'] == 'POST')
	{
		if(isset($_POST['category_id']))
		{
			...
			header("location: list-special.php");
			exit;
		}
	}
	...
	if(isset($_GET['del']))
	{
		...
		$special_model->delete(array('archive_id'=>$archive_id));
		
		header("location: list-special.php#upd");
		exit;
	}
	...
	if($_SERVER["REQUEST_METHOD"] == "POST")
	{
		...
		$special_model->update(array('disp'=>$enable), array('archive_id'=>$archive_id));
		
		header("location: list-special.php?p=".$page_num."#upd");
		exit;
	}
	...
	$special_count = $special_model->getReviewAllCountByFilter($category_id);
	
	//最大ページ数
	$page_max = ceil($special_count / $PAGE_ITEM_COUNT);
	
	$special_all = $special_model->getReviewAllByFilter($category_id, $page_num, $PAGE_ITEM_COUNT);
...
<p class="EntBtn"><a href="reg-special.php">特集ページの登録 &gt;</a></p>

<table class="List">
<tr>
<th class="Date">ジャンル</th>
<td>
<form method="POST" action="list-special.php" id="frmFilter">
...
<td>
<form method="post" action="list-special.php?id=<?php echo $v['archive_id']; ?>&p=<?php echo $page_num; ?>">
...
<td><p class="btn"><a href="reg-special.php?id=<?php echo $v['archive_id']; ?>">編集</a></p></td>
...
<?php for($p=0;$p<$page_max;++$p){ ?>
<?php if($p==$page_num){ ?>
<li><a href="list-special.php?p=<?php echo $p; ?>"><?php echo $p+1; ?></a></li>
<?php }else{ ?>
<li><a href="list-special.php?p=<?php echo $p; ?>"><?php echo $p+1; ?></a></li>
<?php } ?>
<?php } ?>
...
function onDelete(id)
{
	...
	location.href = 'list-special.php?id='+id+"&del";
}
</script>
</body>
</html>
